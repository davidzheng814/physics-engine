...
function render(args, data) {
  ...
  console.log(data.ro_states[0], simulator.bodies.length)
  for (var i = 0; i < data.ro_states[0].length; ++i) {
    for (var j = 0; j < simulator.bodies.length; ++j) {
      var pos = data.ro_states[0][i].pos[j];
      Body.setPosition(simulator.bodies[j], pos);

      if (args.analyzeMean) {
        var pos = data.mean_states[0][i].pos[j];
      } else {
